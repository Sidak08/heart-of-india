      ...
      <main>
        ...
        <section className="band">
          <div className="container">
            <div className="section-head"><div><div className="eyebrow">Something for the table</div><h2 className="section-title">Explore the Menu</h2><p>A small look across our curries, biriyani, tandoori dishes, and combinations.</p></div><Link className="button-secondary" href="/menu">See all {catalogue.items.length} items</Link></div>
            <div className="featured-grid">{featured.map((item) => item && <article className="dish-preview" key={item.id}><span className="eyebrow">{item.categoryId.replaceAll("-", " ")}</span><h3>{item.name}</h3><div className="price">{formatCad(item.priceCents)}</div></article>)}</div>
          </div>
        </section>
