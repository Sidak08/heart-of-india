...
import { setOrderStatus, updateOrderRequestSchema } from "@/lib/server/orders";
...
export async function PATCH(request: Request, context: RouteContext<"/api/operator/orders/[id]">) {
  ...
  try { const order = await setOrderStatus(id, parsed.data); if (!order) return jsonError("This order changed in another window. Refresh and try again.", 409); return Response.json({ order: toOrderView(order) }, { headers: { "Cache-Control": "no-store" } }); }
  catch (error) { return jsonError(error instanceof Error ? error.message : "The order could not be updated.", 503); }
}
