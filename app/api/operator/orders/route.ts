...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ orders: snapshot.orders.map(toOrderView), issues: snapshot.issues, conflictCount: snapshot.conflictCount, checkedAt: new Date().toISOString() }, { headers: { "Cache-Control": "no-store" } });
  }
