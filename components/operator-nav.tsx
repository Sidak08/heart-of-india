...
import { Bell, ClipboardList, ExternalLink, LayoutDashboard, LogOut, Settings, UtensilsCrossed } from "lucide-react";
...
export function OperatorNav({ email }: { email: string }) {
  ...
  async function signOut() { setPending(true); await fetch("/api/operator/logout", { method: "POST" }); router.replace("/operator/login"); router.refresh(); }
