...
export function MenuClient({ categories, menuItems }: { categories: MenuCategory[]; menuItems: MenuItem[] }) {
  ...
  const add = (item: MenuItem, triggerId: string) => { if (item.optionGroups.some((group) => group.required)) { openDetails(item, triggerId); return; } if (cart.addItem(item).ok) markAdded(item); };
  const updateItemQuantity = (item: MenuItem, lineIds: string[], currentQuantity: number, nextQuantity: number) => {
    ...
    if (!target) return;
    cart.updateQuantity(target.lineId, target.quantity + (nextQuantity - currentQuantity));
