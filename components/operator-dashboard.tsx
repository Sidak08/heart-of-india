...
import { AlertTriangle, Banknote, Check, Clock3, Mail, Phone, UserRound } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { playOrderTone } from "@/components/notification-settings";
import { formatCad } from "@/lib/menu";
import { fulfillmentLabels, fulfillmentStatuses, type OrderDataIssue, type StoredOrder } from "@/lib/operations";
...
export function OperatorDashboard({ initialOrders, initialIssues, initialConflictCount, operatorEmail }: { initialOrders: OperatorOrder[]; initialIssues: OrderDataIssue[]; initialConflictCount: number; operatorEmail: string }) {
  ...
  const refresh = useCallback(async (announce = true) => {
    ...
    try {
      ...
      const incoming = data.orders as OperatorOrder[]; const newOrders = incoming.filter((order) => !seen.current.has(order.id)); incoming.forEach((order) => seen.current.add(order.id)); setOrders(incoming); setIssues(Array.isArray(data.issues) ? data.issues : []); setConflictCount(Number.isInteger(data.conflictCount) ? data.conflictCount : 0); setError("");
