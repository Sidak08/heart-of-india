...
export function SiteHeader({ name = "Heart of India", tagline = "Authentic Indian Restaurant", phone = "+19055005382" }: { name?: string; tagline?: string; phone?: string }) {
  ...
  const { itemCount, openCart, cartPulseToken } = useCart();
  return (
    <header className="site-header">
      <div className="tricolour-strip" aria-hidden="true"><span /><span /><span /></div>
      <div className="container header-inner">
        ...
        <button id="site-cart-trigger" className="cart-trigger" type="button" onClick={() => openCart("site-cart-trigger")} aria-label={`Open cart, ${itemCount} items`}><ShoppingBag size={20} /><span className="cart-label">Cart</span><span key={cartPulseToken} className={`cart-count${cartPulseToken ? " cart-count-pulse" : ""}`} aria-hidden="true">{itemCount}</span></button>
