...
export function CartDrawer() {
  ...
  return <><Dialog.Root open={cart.cartOpen} onOpenChange={cart.setCartOpen}><Dialog.Portal><Dialog.Overlay className="overlay" /><Dialog.Content className="cart-drawer" aria-describedby={undefined} onCloseAutoFocus={(event) => { event.preventDefault(); requestAnimationFrame(() => document.getElementById(cart.cartReturnFocusId)?.focus({ preventScroll: true })); }}>
    ...
    <div className="cart-scroll">{cart.lines.length === 0 ? <div className="empty-state"><ShoppingBag size={38} /><h3>Your cart is empty</h3><p>Browse the menu to choose something for pickup.</p><Dialog.Close asChild><Link className="button-primary" href="/menu">View menu</Link></Dialog.Close></div> : cart.lines.map((line) => { const item = cart.catalogue.find((entry) => entry.id === line.itemId); const issue = cart.issues.find((entry) => entry.lineId === line.lineId); return <article className="cart-line" key={line.lineId}><div className="cart-line-title"><div><h3>{line.displayName}</h3>{selectedOptionNames(item, line.selections).map((text) => <p key={text}>{text}</p>)}{issue && <p className="cart-line-warning">{issue.message}</p>}</div><strong>{formatCad(line.unitPriceCents * line.quantity)}</strong></div><div className="cart-line-actions"><QuantityControl value={line.quantity} maximum={Math.min(20, line.quantity + Math.max(0, 50 - cart.itemCount))} onChange={(value) => cart.updateQuantity(line.lineId, value)} label={`${line.displayName} quantity`} /><div>{item?.optionGroups.length && isOrderableItem(item) ? <button className="text-action" type="button" onClick={() => edit({ lineId: line.lineId, item, selections: line.selections, quantity: line.quantity })}>Edit</button> : null}<button className="text-action" type="button" onClick={() => cart.removeLine(line.lineId)}><Trash2 size={17} /> Remove</button></div></div></article>; })}</div>
    {cart.lines.length > 0 && <div className="drawer-summary"><div><span>Subtotal</span><strong>{formatCad(cart.subtotalCents)}</strong></div><p>{cart.canCheckout ? "Tax and the final amount due at the store are shown at checkout." : "Remove or update unavailable items before checkout."}</p><Dialog.Close asChild><Link className="button-primary full" href="/cart">View cart & checkout</Link></Dialog.Close></div>}
