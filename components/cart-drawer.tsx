...
export function CartDrawer() {
  const cart = useCart(); const [editing, setEditing] = useState<Editing | null>(null);
  const edit = (value: Editing) => { cart.setCartOpen(false); requestAnimationFrame(() => setEditing(value)); };
  return <><Dialog.Root open={cart.cartOpen} onOpenChange={cart.setCartOpen}><Dialog.Portal><Dialog.Overlay className="overlay" /><Dialog.Content className="cart-drawer" aria-describedby={undefined} onCloseAutoFocus={(event) => { event.preventDefault(); requestAnimationFrame(() => document.getElementById(cart.cartReturnFocusId)?.focus({ preventScroll: true })); }}>
    ...
    {cart.lines.length > 0 && <div className="drawer-summary"><div><span>Subtotal</span><strong>{formatCad(cart.subtotalCents)}</strong></div><p>Taxes and any configured fees are shown before payment.</p><Dialog.Close asChild><Link className="button-primary full" href="/cart">View cart & checkout</Link></Dialog.Close></div>}
  </Dialog.Content></Dialog.Portal></Dialog.Root>{editing && <ItemDialog item={editing.item} open onOpenChange={(open) => { if (!open) { setEditing(null); requestAnimationFrame(() => cart.setCartOpen(true)); } }} editingLineId={editing.lineId} initialSelections={editing.selections} initialQuantity={editing.quantity} />}</>;
}
